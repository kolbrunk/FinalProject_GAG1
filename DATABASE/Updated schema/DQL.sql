...
where extract(year from M.timest) = 2025 and M.measurement_type = 'Úttekt'
group by 
    P.name,
    extract(year from M.timest),
    extract(month from M.timest),
...
select
    P.name as power_plant_source,
    extract(month from M.timest) as month,
    sum(case when M.measurement_type = 'Framleiðsla' then M.value_kwh else 0 end) as total_production,
    sum(case when M.measurement_type = 'Innmötun' then M.value_kwh else 0 end) as total_import,
    sum(case when M.measurement_type = 'Úttekt' then M.value_kwh else 0 end) as total_withdrawal
from raforka.measurements M
join raforka.power_plants P on P.id = M.power_plant_id
where extract(year from M.timest) = 2025
group by
